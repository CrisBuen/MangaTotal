...
export default function FichaTioanimePage(props: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="space-y-9">
      <Link href="/explorar?seccion=animada&anime_fuente=tioanime" className="inline-block font-mono text-[11px] font-bold tracking-[0.06em] text-subtle transition hover:text-accent-ink">← Explorar anime</Link>
      ...
      <div className="flex flex-col gap-7 sm:flex-row">
        ...
        <div className="min-w-0 flex-1 space-y-5">
          <div>
            <p className="font-mono text-[11px] font-bold tracking-[0.08em] text-accent-ink">TioAnime</p>
          ...
          <div className="flex flex-wrap items-start gap-3">
            ...
            <a href={ficha.url_original} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center rounded-md border border-line bg-panel px-4 font-mono text-[11px] font-bold tracking-[0.06em] text-accent-ink transition hover:border-line-strong">Ver ficha en TioAnime ↗</a>
          </div>
        </div>
      </div>
      ...
      <section>
        <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
          ...
          {ficha.last_page > 1 && <span className="font-mono text-[11px] tracking-[0.06em] text-subtle">Página {ficha.page} de {ficha.last_page}</span>}
        </div>
        ...
        {ficha.episodes.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
            {ficha.episodes.map((episode) => {
              ...
              return (
                <EpisodeWatchLink key={episode.id} href={`/explorar/tioanime/${ficha.slug}/${episode.number}`} className="group overflow-hidden rounded-[10px] border border-line bg-panel transition hover:border-line-strong">
                  ...
                  <div className="p-3">
                    ...
                    {progreso?.completed ? (
                      <p className="mt-2 font-mono text-[11px] font-bold tracking-[0.06em] text-subtle">Ya visto</p>
                    ) : progreso && progreso.position_seconds > 0 ? (
                      <p className="mt-2 font-mono text-[11px] font-bold tracking-[0.06em] text-accent-ink">{minuto(progreso.position_seconds)}</p>
                    ) : null}
                  </div>
                </EpisodeWatchLink>
              );
        ...
        {ficha.last_page > 1 && (
          <div className="mt-7 flex items-center justify-center gap-3">
            <button type="button" disabled={page <= 1} onClick={() => setPage((value) => value - 1)} className="rounded-md border border-line px-4 py-2 font-mono text-[11px] font-bold text-subtle disabled:opacity-40">← Anterior</button>
            <button type="button" disabled={page >= ficha.last_page} onClick={() => setPage((value) => value + 1)} className="rounded-md border border-line px-4 py-2 font-mono text-[11px] font-bold text-subtle disabled:opacity-40">Siguiente →</button>
          </div>
        )}
      </section>

      <p className="border-t border-line pt-6 text-center font-mono text-[11px] tracking-[0.06em] text-subtle">Ficha y episodios provistos por TioAnime con su permiso</p>
    </div>
  );
}
