...
export default function SerieTmoPage(props: {
  params: Promise<{ tipo: string; id: string; slug: string }>;
}) {
  const { tipo, id, slug } = use(props.params);
  const [ficha, setFicha] = useState<Ficha | null>(null);
  const [error, setError] = useState<unknown>(null);
  const [orden, setOrden] = useState<"asc" | "desc">("asc");
  const progreso = useProgresoSerie("tmo", `${tipo}/${id}/${slug}`);
  ...
  return (
    <div className="space-y-8" {...refresco.gesto}>
          ...
          {capitulos.map((c) => (
            <li key={c.id}>
              <Link
                ...
                href={`/leer-externo/tmo/${c.id}?tipo=${tipo}&id=${id}&slug=${ficha.slug}${paginaCapitulo(progreso, String(c.id)) ? "&" + paginaCapitulo(progreso, String(c.id)) : ""}`}
                className={`flex items-center gap-3 px-5 py-3.5 transition hover:bg-[var(--surface-raised)] ${estiloCapitulo(
                  ...
                  capituloLeido(progreso, String(c.id), c.numero)
                )}`}
              >
                <div className="min-w-0 flex-1">
                  ...
                  {(c.grupo || c.fecha) && (
                    <p className="mt-0.5 truncate font-mono text-[11px] tracking-[0.06em] text-subtle">
      ...
      <p className="border-t border-line pt-6 text-center font-mono text-[11px] tracking-[0.06em] text-subtle">
        ...
        <a href={ficha.url_original} target="_blank" rel="noopener noreferrer" className="text-accent-ink hover:underline">
          {TMO_NOMBRE}
        </a>
