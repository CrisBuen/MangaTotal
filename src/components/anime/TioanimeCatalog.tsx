    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          className={`rounded-xl border px-4 py-2.5 font-mono text-[10px] font-bold uppercase tracking-[0.12em] transition ${
            activeFilters ? "border-accent bg-[var(--accent-soft)] text-accent" : "border-line text-subtle hover:text-ink"
          }`}
        >
      ...
      {showFilters && (
        <Surface className="space-y-5 p-5">
          <div>
            <p className="mb-2 font-mono text-[9px] font-bold uppercase tracking-[0.14em] text-subtle">Tipo</p>
              ...
              {TIPOS.map(([value, label]) => (
                <button
                  ...
                  className={`rounded-full border px-3 py-2 text-xs transition ${
                    types.includes(value) ? "border-accent bg-[var(--accent-soft)] text-accent" : "border-line text-subtle hover:text-ink"
                  }`}
                >
                  {label}
                </button>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <label className="space-y-2">
              <span className="block font-mono text-[9px] font-bold uppercase tracking-[0.14em] text-subtle">Estado</span>
              <select value={status} onChange={(event) => setStatus(event.target.value)} className="w-full rounded-xl border border-line bg-[var(--surface-raised)] px-3 py-2.5 text-sm text-ink outline-none focus:border-accent">
                {ESTADOS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
              </select>
            </label>
            <label className="space-y-2">
              <span className="block font-mono text-[9px] font-bold uppercase tracking-[0.14em] text-subtle">Desde el año</span>
              <select value={yearFrom} onChange={(event) => setYearFrom(event.target.value)} className="w-full rounded-xl border border-line bg-[var(--surface-raised)] px-3 py-2.5 text-sm text-ink outline-none focus:border-accent">
                <option value="">Cualquiera</option>
                {[...ANIOS].reverse().map((year) => <option key={year} value={year}>{year}</option>)}
              </select>
            </label>
            <label className="space-y-2">
              <span className="block font-mono text-[9px] font-bold uppercase tracking-[0.14em] text-subtle">Hasta el año</span>
              <select value={yearTo} onChange={(event) => setYearTo(event.target.value)} className="w-full rounded-xl border border-line bg-[var(--surface-raised)] px-3 py-2.5 text-sm text-ink outline-none focus:border-accent">
                <option value="">Cualquiera</option>
          ...
          <div>
            <p className="mb-2 font-mono text-[9px] font-bold uppercase tracking-[0.14em] text-subtle">Géneros</p>
              ...
              {GENEROS_TIOANIME.map(([value, label]) => (
                <button
                  ...
                  className={`rounded-full border px-3 py-1.5 text-xs transition ${
                    genres.includes(value) ? "border-accent bg-[var(--accent-soft)] text-accent" : "border-line text-subtle hover:text-ink"
                  }`}
                >
                  {label}
                </button>
          ...
          {activeFilters > 0 && (
            <button type="button" onClick={limpiar} className="font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-subtle transition hover:text-accent">
              × Limpiar filtros
            </button>
          )}
        </Surface>
      ...
      {error && (
        <Surface className="p-6 text-center">
          ...
          <button type="button" onClick={load} className="mt-3 font-mono text-[10px] uppercase text-accent">Reintentar</button>
        </Surface>
      )}

      {series === null ? (
        <p className="py-16 text-center font-mono text-xs uppercase tracking-[0.14em] text-subtle">Cargando catálogo de TioAnime...</p>
        ...
        <div className="grid grid-cols-2 gap-x-5 gap-y-8 sm:grid-cols-3 lg:grid-cols-5">
          {series.map((anime) => (
            <Link key={anime.slug} href={`/explorar/tioanime/${anime.slug}`} className="group block rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent">
              <div className="relative aspect-[2/3] overflow-hidden rounded-2xl bg-[var(--surface-raised)] ring-1 ring-line transition duration-300 group-hover:-translate-y-1 group-hover:ring-accent group-hover:shadow-[var(--glow)]">
                {anime.cover_url ? (
                  ...
                  <img src={anime.cover_url} alt={anime.title} className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.025]" loading="lazy" referrerPolicy="no-referrer" />
                ) : <div className="grid h-full place-items-center text-xs text-subtle">Sin portada</div>}
              </div>
              <div className="px-1 pt-4">
                <h3 className="line-clamp-2 text-lg font-bold leading-[1.12] text-ink transition group-hover:text-accent">{anime.title}</h3>
                <p className="mt-2 font-mono text-[9px] uppercase tracking-[0.12em] text-subtle">TioAnime</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      {series && series.length > 0 && lastPage > 1 && (
        <div className="flex flex-wrap items-center justify-center gap-3 pt-4">
          <button type="button" disabled={page <= 1} onClick={() => setPage((value) => value - 1)} className="rounded-xl border border-line px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-subtle disabled:opacity-40">← Anterior</button>
          <span className="font-mono text-[10px] uppercase tracking-[0.12em] text-subtle">Página {page} de {lastPage}</span>
          <button type="button" disabled={page >= lastPage} onClick={() => setPage((value) => value + 1)} className="rounded-xl border border-line px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-subtle disabled:opacity-40">Siguiente →</button>
        </div>
      )}

      <p className="border-t border-line pt-6 text-center font-mono text-[10px] uppercase tracking-[0.12em] text-subtle">Catálogo, fichas y episodios provistos por TioAnime con su permiso</p>
    </div>
  );
}
